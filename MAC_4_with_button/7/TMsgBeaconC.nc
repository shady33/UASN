...
module TMsgBeaconC @safe()
{
	uses
	{
		interface Leds;
	    interface Boot;
	    interface Receive as TSReceive_57_1;
	    interface Receive as TSReceive_57_3;
	    interface Receive as TSReceive_25_4_2;

	    interface AMSend as Send_57_2;
	    interface AMSend as Send_57_4;
	    interface AMSend as Send_75_Data; 

	    interface Timer<TMilli> as Timer_57;
	    interface Timer<TMilli> as Timer_57_data;
...
implementation{
	...
	int broadcast = 0, data = 0, counter = 0, cyclesleep7 = 640000, syncsleep7;
	...
	void sleep(uint8_t period)
	{
  	...
  	{
  		data = 1;
  		call Timer_57_data.startOneShot(3640000);
  		//Do nothing.
  	}
    ...
    // do nothing
  	...
  	event void Timer_57_data.fired()
  	{
		data_msg_75 = (data_t*) call Packet.getPayload(&packet_75, sizeof(data_t));
		data_msg_75->src = TOS_NODE_ID;
		data_msg_75->timestamp = call LocalTime.get();
		data_msg_75->data = counter++;

		call Send_75_Data.send(AM_BROADCAST_ADDR, &packet_75, sizeof(data_t));
		//Send data horizontally to node 2.
		sleep(cyclesleep7);
  	}

	event void Timer_57.fired()
	{
		if (broadcast == 1)
		{
			...
			call Send_57_4.send(AM_BROADCAST_ADDR, &bpacket, sizeof(timesync_broad_t));
			broadcast = 0;
			return;
		}
		else
		
		{
			...
			call Send_57_2.send(AM_BROADCAST_ADDR, &packet_75, sizeof(timesync_msg_t));
			call Leds.led0Toggle();
	...
	event message_t* TSReceive_57_1.receive(message_t* bufPtr, void* payload, uint8_t len)
	{
		B1 = call LocalTime.get();
		rcm = (timesync_msg_t*) payload;

		A1 = rcm->local_time;
		send_msg = (timesync_msg_t*)call Packet.getPayload(&packet_75, sizeof(timesync_msg_t));
        if (send_msg == NULL) {
        ...
        call Leds.led0Toggle();
        call Timer_57.startOneShot(10000);
	...
	event message_t* TSReceive_57_3.receive(message_t* bufPtr, void* payload, uint8_t len)
	{
	...
	event void Send_57_2.sendDone(message_t* bufPtr, error_t error) {
		call Leds.led1Toggle();
	}


	event void Send_57_4.sendDone(message_t* bufPtr, error_t error) {
		call Leds.led1Toggle();
	}

	event void Send_75_Data.sendDone(message_t* bufPtr, error_t error) {
		call Leds.led1Toggle();
	}
