...
module TMsgBeaconC @safe()
{
	uses
	{
		interface Leds;
	    interface Boot;
	    interface Receive as TSReceive_24_1;
	    interface Receive as TSReceive_24_3;
	    interface Receive as TSReceive_12_4;

	    interface AMSend as Send_24_2;
	    interface AMSend as Send_24_4;
	    interface AMSend as Send_42_Data; 

	    interface Timer<TMilli> as Timer_24;
	    interface Timer<TMilli> as Timer_24_data;
...
implementation{
	...
	int broadcast = 0, data = 0, counter = 0, cyclesleep4 = 150000, syncsleep4;
	...
	void sleep(uint8_t period)
	{
  	...
  	{
  		data = 1;
  		call Timer_24_data.startOneShot(4020000);
  		//Do nothing.
  	}
    ...
    // do nothing
  	...
  	event void Timer_24_data.fired()
  	{
  			data_msg_42 = (data_t*) call Packet.getPayload(&packet_42, sizeof(data_t));
			data_msg_42->src = TOS_NODE_ID;
			data_msg_42->timestamp = call LocalTime.get();
			data_msg_42->data = counter++;

			call Send_42_Data.send(AM_BROADCAST_ADDR, &packet_42, sizeof(data_t));
	...
	event void Timer_24.fired()
	{
		if (broadcast == 1)
		{
			...
			call Send_24_4.send(AM_BROADCAST_ADDR, &bpacket, sizeof(timesync_broad_t));
			broadcast = 0;
			return;
		}
		else
		
		{
			...
			call Send_24_2.send(AM_BROADCAST_ADDR, &packet_42, sizeof(timesync_msg_t));
			call Leds.led0Toggle();
	...
	event message_t* TSReceive_24_1.receive(message_t* bufPtr, void* payload, uint8_t len)
	{
		B1 = call LocalTime.get();
		rcm = (timesync_msg_t*) payload;

		A1 = rcm->local_time;
		send_msg = (timesync_msg_t*)call Packet.getPayload(&packet_42, sizeof(timesync_msg_t));
        if (send_msg == NULL) {
	...
	event message_t* TSReceive_24_3.receive(message_t* bufPtr, void* payload, uint8_t len)
	{
	...
	event void Send_24_2.sendDone(message_t* bufPtr, error_t error) {
		call Leds.led1Toggle();
	}


	event void Send_24_4.sendDone(message_t* bufPtr, error_t error) {
		call Leds.led1Toggle();
	}

	event void Send_42_Data.sendDone(message_t* bufPtr, error_t error) {
		call Leds.led1Toggle();
	}
