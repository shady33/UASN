...
module TMsgBeaconC @safe()
{
	uses
	{
	    ...
	    interface Timer<TMilli> as Timer_Waitfor6;
        //interface Timer<TMilli> as Timer_Data_56;
...
implementation{
	...
	int stage_56 = 1, stage_57 = 1, cyclesleep5 = 200000, syncsleep5, sync6 = 0, sync7 = 0;
     uint16_t is6=0;uint16_t wait=0; 
     int dyn=0;
	...
	event void AMControl.startDone(error_t err){
		if (err == SUCCESS)
		{
			//make data sending conditional
			//call Timer_Data_52.startOneShot(1050000);
		}
		else
			call AMControl.start();
	}
    ...
    event void Timer_Waitfor6.fired()
    {
  	...
  	event void Timer_56.fired()
  	{
  		if (stage_56 == 1)
  		{
            ...
            call Timer_Data_52.startOneShot(160000);
}
		else if (stage_56 == 2)
		{
			...
			call Timer_56.startOneShot(140000);// This is a repeat timer for the second message to be sent to Node3
	...
	event void Timer_25.fired()
	{
		if (broadcast == 1)
		{
			...
			broadcast = 2;
			call Timer_25.startOneShot(40000);
			return;
		}
		else if (broadcast == 2)
		{
			...
			broadcast = 0;
            if (dyn==0){
			call Timer_56.startOneShot(40000);//(1120 global W.c.)
			dyn = 1;

			}
			else if(dyn==1)
			{
				call Timer_Data_52.startOneShot(40000);
			}
...
//This is for overhearing on 2's final broadcast so as to get the value of the skew/offset of the first hop
	...
	event message_t* DataReceive_65.receive(message_t* bufPtr, void* payload, uint8_t len)
	{
        ...
        dyn=0;
		call Leds.led0Toggle();
		call Timer_Data_52.startOneShot(10000);
		call Timer_56.stop();
