...
module TMsgBeaconC @safe()
{
	uses
	{
        ...
        //interface Timer<TMilli> as Timer_waittillmsg;
...
implementation{
	...
	void sleep(uint8_t period)
	{
  	...
  	{
  		data = 1;
  		//call Timer_56_data.startOneShot(1000000);
  		//Do nothing.
  	}
	...
	event void AMControl.startDone(error_t err){
		...
		{
			data = 1;
  			//call Timer_56_data.startOneShot(1000000);//(23000000)
			//data sending made conditional
			//call Timer_56_data.startOneShot(1000000);
	...
	event void Timer_56.fired()
	{
		if (broadcast == 1)
		{
			...
			//call Timer_waittillmsg.startOneShot(70000);
	...
	event void Send_56_4.sendDone(message_t* bufPtr, error_t error) {
		call Leds.led1Toggle();
		call Timer_56_data.stop();
		call Timer_56_data.startOneShot(70000);
	}
